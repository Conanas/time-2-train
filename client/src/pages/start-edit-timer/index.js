    ...
    const listItems = Object.keys(state).map(key => {
      ...
      if (state[key].title === CONTINUOUS) {
        if (state[key].value === true) {
          inputs = (
            <i className="fa-icon fas fa-toggle-on flow-text" onClick={() => dispatch({ type: state.continuous.action, payload: false })}></i>
          )
        } else {
          inputs = (
            <i className="fa-icon fas fa-toggle-off flow-text" onClick={() => dispatch({ type: state.continuous.action, payload: true })}></i>
          )
        }
      }
